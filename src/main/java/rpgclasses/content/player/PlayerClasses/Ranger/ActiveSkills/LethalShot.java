...
import rpgclasses.projectiles.LethalArrow;
...
public class LethalShot extends ActiveSkill {
    ...
    private static @NotNull Projectile getProjectile(PlayerMob player, PlayerData playerData, int activeSkillLevel) {
        ...
        return new LethalArrow(player.getLevel(), player, player.x, player.y, targetX, targetY, 200, 1000, new GameDamage(DamageTypeRegistry.RANGED, 5 * playerData.getLevel() + 5 * playerData.getStrength(player) * activeSkillLevel), 100);
    }
